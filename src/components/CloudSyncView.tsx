...
import { formatAuthErrorMessage } from '../utils/googleAuth';
...
export const CloudSyncView: React.FC = () => {
  ...
  const handleCopyDomain = async () => {
    try {
      await navigator.clipboard.writeText('instantafrik-rgb.github.io');
      ...
      <div className="p-4 sm:p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs space-y-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 shrink-0">
            {isCloudConnected ? (
              <button
                ...
                disabled={isLoading}
                className="px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 font-medium text-xs flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
              >
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-1.5">
                <button
                  ...
                  disabled={isLoading}
                  className="px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs flex items-center justify-center gap-2 shadow-md transition-all active:scale-98 cursor-pointer"
                  title="Ouvre une fenêtre popup Google (recommandé sur PC, Android Chrome et PWA)"
                >
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="px-3 py-2 rounded-xl border border-indigo-200 dark:border-indigo-800/80 hover:bg-indigo-50 dark:hover:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 font-medium text-[11px] flex items-center justify-center gap-1 transition-colors cursor-pointer"
                  title="Alternative en redirection plein écran si le navigateur bloque les fenêtres popups"
                >
                  Via Redirection
                </button>
              </div>
        ...
        <div className="pt-2.5 border-t border-slate-100 dark:border-slate-800 flex flex-wrap items-center justify-between gap-2 text-xs">
          <div className="text-[11px] text-slate-500 dark:text-slate-400">
            Architecture multi-appareils (PC ↔ Android ↔ PWA) • Domaine autorisé : <span className="font-mono text-indigo-600 dark:text-indigo-400 font-semibold">instantafrik-rgb.github.io</span>
          </div>
          <button
            onClick={() => setShowDiagnostic((prev) => !prev)}
            className="px-3 py-1.5 rounded-lg border border-indigo-200 dark:border-indigo-800 hover:bg-indigo-50 dark:hover:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 font-bold text-xs flex items-center gap-1.5 transition active:scale-95 cursor-pointer"
          >
            <Activity className="w-3.5 h-3.5" />
            {showDiagnostic ? 'Masquer Diagnostic Technique' : 'Ouvrir Diagnostic Technique (GitHub vs Preview)'}
          </button>
        </div>
      </div>

      {/* Alerte Explicite d'Erreur Firebase Authentication avec Diagnostic en Direct */}
      {lastAuthError && !isCloudConnected && (
        <div className="p-4 rounded-2xl bg-rose-50 dark:bg-rose-950/60 border-2 border-rose-300 dark:border-rose-800 space-y-3 text-xs shadow-sm">
          <div className="flex items-start gap-3 text-rose-900 dark:text-rose-200">
            ...
            <div className="space-y-1.5 flex-1">
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-bold text-sm">Échec de connexion Google Authentication</span>
                <code className="font-mono text-[11px] bg-rose-200 dark:bg-rose-900/80 px-2 py-0.5 rounded text-rose-900 dark:text-rose-100 font-bold">
                  {lastAuthError.code}
                </code>
              </div>
              <p className="text-xs text-rose-800 dark:text-rose-300 leading-relaxed font-medium">
                {formatAuthErrorMessage(lastAuthError)}
              </p>

              {lastAuthError.code === 'auth/unauthorized-domain' && (
                <div className="mt-2.5 p-3.5 bg-white dark:bg-neutral-900 rounded-xl border border-rose-200 dark:border-rose-900/80 space-y-2.5">
                  <p className="font-bold text-xs text-neutral-900 dark:text-white">
                    Action requise dans Firebase Console pour autoriser ce domaine :
                  </p>
                  <ol className="list-decimal list-inside space-y-1 text-[11px] text-neutral-600 dark:text-neutral-300">
                    <li>Ouvrez la <strong>Console Firebase</strong> et cliquez sur votre projet <strong>nantor-sourcing-prod</strong></li>
                    <li>Dans le menu gauche, allez sur <strong>Authentication</strong> &gt; onglet <strong>Paramètres (Settings)</strong></li>
                    <li>Faites défiler jusqu'à la section <strong>Domaines autorisés (Authorized domains)</strong></li>
                    <li>Cliquez sur <strong>Ajouter un domaine</strong> et collez exactement le domaine ci-dessous :</li>
                  </ol>
                  <div className="flex items-center gap-2 pt-1">
                    <code className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 font-mono text-indigo-700 dark:text-indigo-300 font-bold text-xs border border-slate-200 dark:border-slate-700 select-all">
                      instantafrik-rgb.github.io
                    </code>
                    <button
                      onClick={handleCopyDomain}
                      className="px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs flex items-center gap-1.5 cursor-pointer transition active:scale-95 shadow-xs"
                    >
                      {copiedDomain ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                      {copiedDomain ? 'Domaine copié !' : 'Copier'}
                    </button>
                  </div>
                  <div className="p-2.5 bg-indigo-50 dark:bg-indigo-950/40 rounded-lg border border-indigo-200 dark:border-indigo-800 text-[11px] text-indigo-900 dark:text-indigo-200 space-y-1">
                    <p className="font-bold">
                      💡 Le bouton "Ajouter un domaine" est grisé ou absent pour ce projet Starter ?
                    </p>
                    <p>
                      Ouvrez le <strong>Diagnostic Technique</strong> ci-dessous : vous pouvez y connecter votre propre projet Firebase gratuit en 30 secondes pour une autonomie totale sur GitHub Pages, Android PWA et Windows PWA.
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-rose-200 dark:border-rose-900">
            <button
              onClick={() => handleGoogleSignIn(false)}
              disabled={isLoading}
              className="px-3 py-1.5 rounded-lg bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs flex items-center gap-1.5 cursor-pointer shadow-xs transition active:scale-95"
            >
            ...
            <button
              onClick={() => handleGoogleSignIn(true)}
              disabled={isLoading}
              className="px-3 py-1.5 rounded-lg border border-rose-300 dark:border-rose-700 hover:bg-rose-100 dark:hover:bg-rose-900 text-rose-800 dark:text-rose-200 font-semibold text-xs flex items-center gap-1.5 cursor-pointer transition active:scale-95"
            >
              Tenter par Redirection plein écran
            </button>
          </div>
        </div>
      )}

      {/* Panneau de Diagnostic Technique Production */}
      {showDiagnostic && <ProductionDiagnosticPanel />}
